/**
 * x402-news-wire — agent client example.
 *
...
async function main(): Promise<void> {
  ...
  const unpaid = await fetch(`${BASE_URL}/query?q=semiconductor%20export%20controls&timespan=3d&maxRecords=25`);
  ...
  {
    const res = await payFetch(`${BASE_URL}/query?q=semiconductor%20export%20controls&timespan=3d&maxRecords=25`);
    if (!res.ok) {
      console.error(`GET /query failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /query ($0.003) ===");
  ...
  {
    const res = await payFetch(`${BASE_URL}/pulse?q=semiconductor%20export%20controls&cursor=20260806T000000Z&maxRecords=50`);
    if (!res.ok) {
      console.error(`GET /pulse failed: ${res.status}`, await res.text());
      process.exit(1);
    }
    console.log("=== GET /pulse ($0.002) ===");
    console.log(JSON.stringify(await res.json(), null, 2));
